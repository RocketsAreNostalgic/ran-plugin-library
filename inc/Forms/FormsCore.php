<?php
/**
 * FormsCore: Shared functionality for form-based classes.
 *
...
abstract class FormsCore implements FormsInterface {
	...
	protected function _handle_submit_controls_zone_update(array $data): void {
		...
		if ($container_id === '' || $zone_id === '') {
			$this->logger->warning('FormsCore: Submit controls zone update missing required IDs', array(
	...
	protected function _handle_submit_controls_set_update(array $data): void {
		...
		if ($container_id === '' || $zone_id === '') {
			$this->logger->warning('FormsCore: Submit controls set update missing required IDs', array(
				'container_id' => $container_id,
				'zone_id'      => $zone_id,
			));
			return;
	...
	protected function _handle_section_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '') {
			$this->logger->warning('FormsCore: Section update missing required IDs', $data);
			return;
		}
	...
	protected function _handle_section_metadata_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '') {
			$this->logger->warning('FormsCore: Section metadata update missing required IDs', $data);
			return;
		}

		if (!$this->_get_state_store()->has_section($container_id, $section_id)) {
			$this->logger->warning('FormsCore: Section metadata update received before section registration', $data);
			return;
		}
	...
	protected function _handle_field_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '') {
			$this->logger->warning('FormsCore: Field update missing required IDs', $data);
			return;
		}
	...
	protected function _handle_group_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '' || $group_id === '') {
			$this->logger->warning('FormsCore: Group update missing required IDs', $data);
			return;
		}
	...
	protected function _handle_group_field_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '' || $group_id === '') {
			$this->logger->warning('FormsCore: Group field update missing required IDs', $data);
			return;
		}
	...
	protected function _handle_group_metadata_update(array $data): void {
		...
		if ($container_id === '' || $section_id === '' || $group_id === '') {
			$this->logger->warning('FormsCore: Group metadata update missing required IDs', $data);
			return;
		}
	...
	protected function _handle_section_cleanup(array $data): void {
		...
		if ($section_id === '') {
			$this->logger->warning('FormsCore: Section cleanup missing section_id', $data);
			return;
		}
	...
	protected function _handle_template_override(array $data): void {
		...
		if (!is_array($overrides)) {
			$this->logger->warning('FormsCore: Template override overrides must be array', $data);
			return;
		}
	...
	protected function _handle_form_defaults_override(array $data): void {
		...
		if (empty($overrides) || !is_array($overrides)) {
			$this->logger->warning('FormsCore: Form defaults override missing or invalid overrides', $data);
			return;
		}
	...
	protected function _handle_custom_update(string $type, array $data): void {
		$this->logger->warning('FormsCore: Unknown update type received', array(
			'type'      => $type,
