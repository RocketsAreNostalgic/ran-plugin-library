<?php
/**
 * BlockFactory Fluent Interface Usage Example
 *
 * This example demonstrates how to use the new BlockFactory fluent interface
 * wrapper around BlockRegistrar for improved developer experience.
 *
...
function my_plugin_render_hero_block(array $attributes, string $content): string {
	...
	return sprintf(
		'<div class="hero-block"><h2>%s</h2><div class="hero-content">%s</div></div>',
		$title,
		$content
	);
}
...
/**
 * Example render callback for admin block.
 *
 * @param array<string, mixed> $attributes Block attributes.
 * @param string               $content    Block content.
 * @return string
 */
function my_plugin_render_admin_block(array $attributes, string $content): string {
	if (!is_admin()) {
		return '';
	}

	return '<div class="admin-only-block">This block only appears in admin context.</div>';
}
