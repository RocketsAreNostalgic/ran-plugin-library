...
trait ScriptsEnqueueTrait {
	...
	protected function _process_script_extras(array $asset_definition, string $handle, ?string $hook_name): void {
		...
		if (null === $hook_name && $handle) {
			...
			if (!empty($custom_attributes)) {
				...
				$this->_get_hooks_manager()->register_filter('script_loader_tag', $callback, 10, 2, array());
			}
		}
	}
	...
	protected function _modify_html_tag_attributes(
		...
		if ($logger->is_active()) {
			$logger->debug("{$context} - Successfully modified tag for '{$handle_to_match}'. New tag: {$modified_tag}");
		}
		return $modified_tag;
	}
}
