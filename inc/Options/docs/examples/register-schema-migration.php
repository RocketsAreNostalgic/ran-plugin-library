...
if (version_compare($current_version, '2.0.0', '<')) {
	...
	$options->register_schema(array(
	    'analytics_enabled' => array(
	        ...
	        'validate' => fn($v) => Validate::basic()->isBool($v),
	    ),
	    'analytics_tracking_id' => array(
	        'default'  => '',
	        'sanitize' => fn($v) => sanitize_text_field($v),
	        'validate' => fn($v) => Validate::basic()->isString($v),
	    ),
'plugin_version' => array(
		'default'  => '2.0.0',
		'validate' => fn($v) => Validate::basic()->isString($v) && !empty($v),
	    ),
	));
}
...
'image_quality' => array(
		'default'  => 85,
		'validate' => fn($v) => Validate::basic()->isInt($v) && $v >= 1 && $v <= 100,
	    ),
...
$options->register_schema(array(
    ...
    'test_group' => array(
        'default'  => $test_group,
        'validate' => fn($v) => Validate::basic()->isInt($v) && in_array($v, array(0, 1)),
	)
)
);
...
add_action('plugins_loaded', function() {
	...
	if (version_compare($db_version, '1.1', '<')) {
		$options->register_schema(array(
			'cache_enabled'  => array('default' => true, 'validate' => fn($v) => Validate::basic()->isBool($v)),
			'cache_duration' => array('default' => 3600, 'validate' => fn($v) => Validate::basic()->isInt($v) && $v > 0),
			'db_version'     => array('default' => '1.1', 'validate' => fn($v) => Validate::basic()->isString($v) && $v !== ''),
		));
	}
	...
	if (version_compare($db_version, '1.2', '<')) {
		$options->register_schema(array(
			'api_endpoint' => array('default' => 'https://api.example.com/v1', 'validate' => fn($v) => Validate::basic()->isString($v) && $v !== ''),
			'api_timeout'  => array('default' => 30, 'validate' => fn($v) => Validate::basic()->isInt($v) && $v > 0),
			'db_version'   => array('default' => '1.2', 'validate' => fn($v) => Validate::basic()->isString($v) && $v !== ''),
		));
	}
