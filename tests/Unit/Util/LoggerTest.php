...
class LoggerTest extends PluginLibTestCase {
	 ...
	 * @test
	 */
	public function test_wp_debug_and_manage_options_activate_debug_level(): void {
		...
		$this->assertTrue($logger->is_active(), 'Logger should be active when WP_DEBUG(_LOG) is enabled and the current user can manage_options.');
		$this->assertSame(Logger::LOG_LEVELS_MAP[LogLevel::DEBUG], $logger->get_log_level(), 'Log level should be DEBUG when logger is active.');
	}

	/**
	 * @test
	 */
	public function test_logger_does_not_activate_when_user_cannot_manage_options(): void {
		WP_Mock::userFunction('current_user_can', array(
			'return' => false,
		));
		$config = array(
		...
		$this->assertFalse($logger->is_active(), 'Logger should not be active when current user cannot manage_options.');
		$this->assertSame(0, $logger->get_log_level(), 'Log level should be 0 when logger is inactive.');
	}

	/**
	 * @test
	 */
	public function test_constant_empty_string_does_not_activate_logger(): void {
		...
		$this->assertFalse($logger->is_active(), 'Logger should not be active based on constants.');
		$this->assertSame(0, $logger->get_log_level(), 'Log level should be 0 when logger is inactive.');
	}
	 ...
	 */
	public function test_constant_valid_level_name_does_not_activate_logger(): void {
	 ...
	 */
	public function test_url_parameter_valid_level_name_does_not_activate_logger(): void {
		$param_name        = self::DEFAULT_CONFIG['debug_request_param'];
		$_GET[$param_name] = LogLevel::INFO;
		$logger            = new Logger(self::DEFAULT_CONFIG);
		$this->assertFalse($logger->is_active(), 'Logger should not be active based on URL parameters.');
		$this->assertSame(0, $logger->get_log_level(), 'Log level should be 0 when logger is inactive.');
	 ...
	 */
	public function test_url_parameter_numeric_level_does_not_activate_logger(): void {
		$param_name        = self::DEFAULT_CONFIG['debug_request_param'];
		$_GET[$param_name] = (string) Logger::LOG_LEVELS_MAP[LogLevel::WARNING];
		$logger            = new Logger(self::DEFAULT_CONFIG);
		$this->assertFalse($logger->is_active(), 'Logger should not be active based on URL parameters.');
		$this->assertSame(0, $logger->get_log_level(), 'Log level should be 0 when logger is inactive.');
		unset($_GET[$param_name]);
	 ...
	 */
	public function test_url_parameter_true_value_does_not_activate_logger(): void {
		$param_name        = self::DEFAULT_CONFIG['debug_request_param'];
	...
	public function test_log_method_does_not_log_when_level_below_effective_level(): void {
		...
		$this->assertCount(2, $this->logged_messages, 'Expected messages to be logged when logger is active.');
	}
	 ...
	 */
	public function test_constant_does_not_activate_logger_when_url_param_is_invalid_and_present(): void {
		$url_param_name = 'TEST_INVALID_URL_VALID_CONST_PARAM';
		$constant_name  = 'TEST_INVALID_URL_VALID_CONST_CONST';

		$_GET[$url_param_name] = 'INVALID_LEVEL';
		...
		$this->assertFalse($logger->is_active(), 'Logger should not be active based on URL parameters or constants.');
		$this->assertSame(0, $logger->get_log_level(), 'Log level should be 0 when logger is inactive.');
