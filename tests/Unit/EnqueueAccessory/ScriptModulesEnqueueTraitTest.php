...
class ScriptModulesEnqueueTraitTest extends EnqueueTraitTestCase {
	...
	public function test_deregister_with_single_handle_string_immediate(): void {
		...
		$result = $this->instance->deregister([['handle' => $handle, 'immediate' => true]]);
	...
	public function test_deregister_with_deferred_behavior(): void {
		...
		$result = $this->instance->deregister([
			['handle' => $handle, 'hook' => $hook, 'priority' => $priority]
		]);
	...
	public function test_enqueue_deferred_modules_processes_deferred_assets(): void {
		...
		$deferred_assets = [$hook_name => [$priority => [$deferred_asset]]];
		...
		WP_Mock::userFunction('wp_register_script_module')
			->with($handle, 'path/to/deferred.js', [], '1.0')
			->once()
			->andReturn(true);
		...
		$this->assertEmpty($remaining_deferred[$hook_name][$priority] ?? []);
	}
	...
	public function test_enqueue_deferred_modules_with_no_matching_assets(): void {
		...
		$this->set_protected_property_value($this->instance, 'deferred_assets', []);
	...
	public function test_enqueue_deferred_modules_with_multiple_assets_same_priority(): void {
		...
		$assets = [
			...
			[
				...
				'deps' => ['@my-plugin/module-1'],
				'version' => '2.0'
			]
		];

		// Set up deferred assets
		$deferred_assets = [$hook_name => [$priority => $assets]];
		$this->set_protected_property_value($this->instance, 'deferred_assets', $deferred_assets);

		// Mock WordPress functions for both modules
		WP_Mock::userFunction('wp_register_script_module')
			->with('@my-plugin/module-1', 'path/to/module-1.js', [], '1.0')
			->once()
			->andReturn(true);
		WP_Mock::userFunction('wp_enqueue_script_module')
			...
			->once();
		WP_Mock::userFunction('wp_register_script_module')
			->with('@my-plugin/module-2', 'path/to/module-2.js', ['@my-plugin/module-1'], '2.0')
			->once()
			->andReturn(true);
		WP_Mock::userFunction('wp_enqueue_script_module')
